...
var point = (function () {
    ...
    that.init = function () {
        that.filePath = window.location.href.indexOf("localhost") > -1 ?
            "https://cand3.sharepoint.com/Shared%20Documents/POC%20TO/Copy%20To%20Folder/Folder%20in%20POC/Folder%202/TestWord.docx" : Office.context.document.url;
        that.controls = {
            body: $("body"),
            main: $(".main"),
            back: $(".n-back"),
    ...
    that.action = {
        ...
        choose: function (o) {
            ...
            if (!that.selected || (that.selected && that.selected.Id != _i)) {
                that.selected = { Id: _i, File: _selectedItem.data("file"), Name: _selectedItem.data("name"), Value: app.string(_s.Value), SourceType: _s.SourceType };
            ...
            if (that.firstLoad || options.refresh) {
                ...
                if (options.index < options.data.length) {
                    var _dsp = options.data[options.index],
                        ...
                        _index = that.range.index({ data: options.tags, tag: _tag }),
                        _pvt = _sp.PublishedHistories && _sp.PublishedHistories.length > 0 ? (_sp.PublishedHistories[0].Value ? _sp.PublishedHistories[0].Value : "") : "";
                    if (_sp.SourceType == app.sourceTypes.table && _dsp.DestinationType == app.destinationTypes.all) {
                        options.data[options.index].IsPreviousTable = true;
                        handleError();
                    }
                    else {
                        var _pv = _sp.SourceType == app.sourceTypes.table ? (_dsp.DestinationType == app.destinationTypes.tableImage ? JSON.parse(_pvt).image : JSON.stringify(JSON.parse(_pvt).table)) : _pvt;
                        Word.run(function (ctx) {
    ...
    that.ui = {
        ...
        sources: function (options) {
            ...
            if (_st.sortType == app.sortTypes.name) {
                ...
                if (_st.sortOrder == app.sortOrder.asc) {
                    _d.sort(function (_a, _b) {
                        return (app.string(_a.Value).toUpperCase() > app.string(_b.Value).toUpperCase())
                            ? 1 : (app.string(_a.Value).toUpperCase() < app.string(_b.Value).toUpperCase()) ? -1 : 0;
                    });
                }
                else {
                    _d.sort(function (_a, _b) {
                ...
                that.range.values({ data: $.extend([], that.points), tags: _tags, refresh: options.refresh }, function (result) {
                    ...
                    if (_st.sortType == app.sortTypes.name) {
                        ...
                        if (_pt == app.sourceTypes.point) {
                            if (_st.sortOrder == app.sortOrder.asc) {
                                _df.sort(function (_a, _b) {
                                    return (app.string(_a.ReferencedSourcePoint.Value).toUpperCase() > app.string(_b.ReferencedSourcePoint.Value).toUpperCase())
                                        ? 1 : (app.string(_a.ReferencedSourcePoint.Value).toUpperCase() < app.string(_b.ReferencedSourcePoint.Value).toUpperCase()) ? -1 : 0;
                                });
                            }
                            else {
                                _df.sort(function (_a, _b) {
            ...
            if (options.index < options.data.length) {
                ...
                if (options.index >= that.pagerSize * (that.pagerIndex - 1) && options.index < that.pagerSize * that.pagerIndex) {
                    ...
                    $.each(_pht, function (m, n) {
                        var __cv = app.string(n.Value), __pvr = _sourceType == app.sourceTypes.point ? __cv : (__cv == "Cloned" ? "Cloned" : "Published");
                    ...
                    _h += '<div class="e2">';
                    if (_dsp.IsPreviousTable) {
                        _h += '<p><strong>Error</strong>: Old source tables could not be recognized in this version. Please delete and recreate them in Excel and add them in Word again.</p>';
                    }
                    else {
                        if (!_fd) {
        ...
        customFormat: function (options, callback) {
            ...
            var _v = options.ref ? app.string(options.selected.ReferencedSourcePoint.Value) : app.string(options.selectedPoint.Value); //_dataList.find("li." + _itemSelectedName + " .btnSelectFormat").prop("original");
